...
7. **AI Security / AI Grid** — a separate, entitlement-gated (`ai.security`) pipeline discovering *other systems'* AI/ML resources (not VulnWatch's own AI-assist features). Per completed scan: immutable snapshot → derived facts → ownership resolution → agent-rooted system grouping → policy assessment (JSON predicate engine) → R2 cross-system exposure correlation (promoted to "validated" only on exact, fresh evidence) → coverage/setup-action reconciliation → bridge into the canonical `findings` table (`finding_kind = AI_POSTURE`/`AI_EXPOSURE`/`AI_RUNTIME`). Assessment decisions are projected into the customer vocabulary `PASS`/`FAIL`/`UNKNOWN`/`NOT_ASSESSED`; `OUT_OF_SCOPE`, `CAPABILITY_UNAVAILABLE`, and `NOT_APPLICABLE` are `NOT_ASSESSED`, while incomplete/stale/error evidence is `UNKNOWN`. Azure Foundry, Azure ML, and Copilot collectors also populate bounded, metadata-only runtime execution records; no raw/high-volume provider logs, prompts, responses, or tool payloads are ingested. A bounded runtime evaluator (`AiGridRuntimeEvaluationService`, modes `RUNTIME_FACTS`/`RUNTIME_SEQUENCE`/`RUNTIME_AGGREGATE`/`RUNTIME_COVERAGE`) runs the `AGCF-RT-001`..`009` preview policy pack over those execution/event records and reconciles `FAIL`s onto `finding_kind = AI_RUNTIME`; a governed customer-telemetry adapter (`POST /api/internal/ai-grid/runtime/{producerId}/v1/batches`) admits the same shaped evidence from frameworks/gateways when a provider connector can't supply it. A separate platform-owner governance track (answer-key precision review, release certification, GA/canary policy distribution) gates what content reaches tenants. Tenant framework coverage, latest assessment-state totals, and the Program 2 runtime-telemetry gate render on `/policies`; tenant execution metadata renders on `/inventory/ai/executions`; `/platform/ai-policies` contains global catalog/distribution controls only. Full detail: `docs/business-logic-guide.md#ai-security--ai-grid-pipeline`.
...
| `/inventory/:inventoryView?` | `InventoryPage` (dispatches sub-views) | Default `overview`; `ai` sub-view shows `AiInventoryPage` |
...
| `/inventory/ai/assets`, `/inventory/ai/knowledge-data`, `/inventory/ai/mcp`, `/inventory/ai/executions`, `/inventory/ai/:assetId` | `AiInventoryAssetsPage`, `AiKnowledgeMcpInventoryPage`, `AiAgentExecutionsPage`, `AiAssetDetailPage` | AI artifact inventory and bounded runtime metadata — entitlement-gated (`ai.security`) |
| `/findings/ai`, `/findings/ai/:findingId` | `AiFindingsPage`, `AiFindingDetailPage` | Deterministic AI policy-failure findings — separate stream from CVE findings |
| `/findings/ai/exposures[/:exposureId]` | `AiExposuresPage` | AI Grid cross-system validated exposure paths |
| `/policies[/:policyId]` | `AiPoliciesPage`, `AiPolicyDetailPage` | Tenant AI Grid policy catalog — enable/disable plus tenant framework coverage, assessment-state totals, runtime-telemetry readiness, scope, exceptions, and parameters |
...
| `/platform/:platformView?` | `PlatformConsolePage` | Default `tenants`; platform-owner console. `ai-policies` sub-view is `PlatformAiPolicyStudio` (global governed AI Grid catalog/distribution/rollout; tenant coverage and telemetry are intentionally not rendered here) |
...
- `src/features/ai-security/types.ts` — AI Security/AI Grid artifact, finding, policy, coverage, and exposure-intelligence types
...
- **Per-tenant schema DDL** (every table that lives inside `tenant_default`/`tenant_<id>`, including RLS policy changes): create `backend/src/main/resources/db/migration/tenant/V{next}__description.sql`, beginning with `-- migration-guard: tenant-only`. This line is **not** run by the application's startup Flyway — it is applied once per tenant schema by `TenantSchemaMigrationService` (dev/test) or `ProductionBootstrapCli` (production), which migrate the `tenant_default` template first, verify a structural fingerprint, then roll out to a canary tenant and the rest in batches of 10. Files may use the `${tenantId}`/`${tenantSchema}` Flyway placeholders. **Current heads: platform `postgres_reset/V3`, tenant `tenant/V3`** — platform V3 registers the bounded runtime evaluation modes and policy-addressable runtime fields; tenant V3 adds the typed runtime contract, governed producer/source registry, receipts, and rolling quota state. `MigrationCatalogTest` asserts both numbers, so a new migration must bump it in the same PR. Unlike the rest of the codebase, AI Grid tables have no JPA entity/repository (accessed via `JdbcTemplate` in `aisecurity.service`). See `docs/database.md#tenant-schema-control-plane` and `docs/database.md#ai-security--ai-grid-tables` for the full mechanics.
...
| Every 1 min (default) | Recover stale `PROCESSING` finding-delta-queue entries (sibling of the 2s drain, recovers from a worker dying mid-batch) |
| Every 30 min (default) | Entitlement shadow-mode sweep — recomputes corrected-vs-legacy entitlement decisions per tenant/key |
| Every 30 sec (default) | Demo dataset provisioning poll — seeds/repairs requested tenants' demo data once `ACTIVE` |
| Every 3 sec (default) | Poll `ingestion_jobs` for AI Security AWS/Azure discovery jobs |
| Every 2 sec (default) | Poll `ingestion_jobs` for AI Grid runtime telemetry-adapter batches (`AiGridRuntimeIngestionWorker`) |
...
- **Migrations**: `V<next>__short_snake_case.sql` in the appropriate independent line: `backend/src/main/resources/db/migration/postgres_reset/` for platform/public DDL or `backend/src/main/resources/db/migration/tenant/` for tenant-schema DDL.
