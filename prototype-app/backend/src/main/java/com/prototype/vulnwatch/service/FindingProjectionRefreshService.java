...
public class FindingProjectionRefreshService {
    ...
    public int refreshTenant(Tenant tenant) {
        Integer refreshed = writeTransactionTemplate.execute(status -> refreshTenantInternal(tenant));
    ...
    private int refreshTenantInternal(Tenant tenant) {
                ...
                jdbcTemplate.update("""
                        ...
                        SELECT
                            ...
                            EXISTS (
                                ...
                                WHERE upper(fix_record.cve_id) = upper(vulnerability.external_id)
                                  AND upper(cast(fix_record.fix_type as varchar)) <> 'NO_FIX'
                            ),
