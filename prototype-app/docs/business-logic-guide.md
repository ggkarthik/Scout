...
Microsoft Copilot Studio discovery uses the same durable ingestion path as AWS and Azure. `POST /{connectorId}/run` and scheduled discovery both enqueue an `AI_SECURITY_COPILOT_STUDIO` job; `AiSecurityJobWorkerService` claims it under the provider concurrency limit and dispatches `CopilotStudioDiscoveryProvider`. The worker—not the request thread—starts and completes the `SyncRun`, applies the connector kill switch immediately before collection, and records terminal job state for queue visibility. The discovery service also records a tenant budget admission before its Dataverse metadata sweep. It emits the same `ObservationEnvelopeV1` (`provider = MICROSOFT_COPILOT`) into `AiSecurityObservationService` as the other providers.
...
4. **Policy assessment** (`AiGridAssessmentService`) — evaluates each artifact against every policy the tenant has selected REQUIRED/ENABLED/PREVIEW for (via `AiGridPredicateEngine`, a JSON predicate evaluator over facts, with per-tenant scope conditions/exceptions/parameters), producing PASS/FAIL/NO_DECISION/ERROR/NOT_APPLICABLE persistence decisions. Customer reporting derives four states from those values: PASS and FAIL are direct; OUT_OF_SCOPE, CAPABILITY_UNAVAILABLE, and NOT_APPLICABLE become NOT_ASSESSED; incomplete/stale evidence and collection errors become UNKNOWN. The derivation lives in `AiGridFindingService.AssessmentResult.deriveAssessmentState` and is reused by the latest per-policy state-summary API.
...
- **Portfolio** (`AiGridPolicyPortfolioService`) — reads the registered OWASP GenAI LLM Top 10 2026, OWASP Agentic Top 10 2026, and CSA AICM 1.1 control sets; computes tenant breadth/effective coverage for current or retained historical epochs; and maintains a RICE-scored intake backlog. Package grouping uses `release_family`, not an `AGCF-` naming convention. A mapping counts as effective only when its policy is distributed to the tenant, REQUIRED/ENABLED, applicable, and decision-ready; PREVIEW is visible but excluded from effective coverage.
...
### Phase 1 Coverage Expansion Status

The infrastructure slice is implemented, but governed content review remains deliberately fail-closed:

- Platform V2 registers framework versions/controls, typed runtime predicate fields, and one reusable tenant-visibility rule (available + GA, or tenant-listed CANARY/DEV). Catalog, selection, and coverage queries use that rule.
- Framework coverage separates mapped breadth from effective coverage, reports PREVIEW independently, includes applicable and decision-ready counts, emits PARTIAL where appropriate, and returns mapping type/rationale/selection/readiness/distribution detail. A paused policy is breadth without effective coverage and carries `POLICY_PAUSED_PENDING_EVIDENCE` rather than a fictional per-subject assessment.
- New mappings use `DIRECT`, `PARTIAL`, or `SUPPORTING`; legacy `INFORMATIVE` values remain readable as supporting compatibility data but should not be authored into new package versions. Mapping-level version comparison—not package-level uniqueness—is the intended grandfathering boundary.
- Runtime predicates are allowed to reference only fields in `platform.ai_grid_runtime_field_definitions`; JSONB diagnostics/extensions are not predicate inputs.
- Tenant V2 adds approved agent-version manifests and component allowlists. Approve/revoke writes and the bulk-enable operation are admin-only sensitive actions. Bulk enable changes only effective selections, audits once, and refreshes projections only when data changed.
- Release compilation/validation resolves columns by COPY header, preserves existing package/distribution digests, and validates the legacy parity artifact structurally. The 21 live legacy policies are not certified for retirement until each row has an explicit successor or no-successor disposition plus evidence, fingerprint, and selection capture; the validator currently reports them as installed but unverified rather than guessing parity.

Program 1 exit also requires tenant V2 rollout completion for Wave A tenants. This is separate from the Program 2 entry gate below.
...
Runtime execution retention is separate. `AiAgentExecutionRetentionService` deletes expired `ai_agent_executions` parents in batches (their events use `ON DELETE CASCADE`), bounded to 100 batches or 30 seconds per tenant per sweep. It returns deleted/remaining/batch counts and logs a warning when eligible rows remain, preventing one long transaction over a large event set.

### Program 2 Entry Gate

Program 2 builds on bounded, metadata-only execution/event records, not raw CloudWatch/application log ingestion. Azure Foundry, Azure ML, and Microsoft Copilot collection already use the shared ingestion boundary, which HMACs provider identifiers, enforces event/metadata-size limits, maintains overlap-aware cursors, and deduplicates through receipts. Tenants can inspect the resulting records at `/inventory/ai/executions`; this foundation does not itself mean the Program 2 operating gates have passed.

`AiGridRuntimeTelemetryReadinessService.readiness()` enumerates evidence sources from configuration — `ai_runtime_source_configurations` (seeded `AZURE_FOUNDRY_RUNTIME`, `COPILOT_STUDIO_RUNTIME`, both `PROVIDER_CONNECTOR`) unioned with registered `ai_runtime_evidence_producers` (`TELEMETRY_ADAPTER`) — rather than a fixed two-provider list, so any newly registered adapter producer reports its own row. The gate itself still requires **two passing `PROVIDER_CONNECTOR` sources**; adapter rows are reported and certified separately and never substitute for a provider connector in the overall gate.

Each provider connector is evaluated over a rolling 14-day tenant window and must independently reach: at least 100 executions; at least 95% populated `approval_state`, `policy_state`, and `action_outcome` **measured over consequential events only** — those whose `action_category` is `WRITE`, `DELETE`, `SEND`, `EXECUTE`, `ADMIN`, `PAYMENT`, or `PUBLISH`; at least 90% execution-to-agent correlation; and at least 80% version correlation over a **version-applicable sample of at least 50 executions** (below that floor the criterion reports `VERSION_SAMPLE_INSUFFICIENT` rather than passing vacuously on a zero denominator). Execution-level fill rates are still computed and returned for continuity, but restricting the decision metrics to consequential events is the point of the change — an execution-level fill rate can read 100% while no consequential action carries a decision. An unconfigured provider reports `NOT_CONFIGURED`; a configured-but-insufficient one reports the specific blocker (`MINIMUM_EXECUTIONS_NOT_MET`, `CONSEQUENTIAL_EVENT_SAMPLE_INSUFFICIENT`, `APPROVAL_STATE_FILL_BELOW_THRESHOLD`, `POLICY_STATE_FILL_BELOW_THRESHOLD`, `ACTION_OUTCOME_FILL_BELOW_THRESHOLD`, `AGENT_CORRELATION_BELOW_THRESHOLD`, `VERSION_SAMPLE_INSUFFICIENT`, `VERSION_CORRELATION_BELOW_THRESHOLD`, or `TENANT_QUOTA_EXHAUSTED`). Duplicate, quarantine, lag, late-event, and out-of-order metrics remain Program 2 ingestion/operations deliverables rather than entry-gate inputs.

Tenant V2 supplies the approved agent-version manifest and component-allowlist control plane needed by runtime policies; tenant V3 supplies the typed runtime contract, the producer/source registry, receipts, and quotas the gate and the evaluator both depend on. Both are rolled out per tenant through `TenantSchemaMigrationService`/`ProductionBootstrapCli` (template fingerprint → canary → batches of 10), not created by the normal application deploy. A tenant below the required version is not an assessment error; readiness should expose a distinct schema blocker and leave dependent controls not assessed.

### Phase 2 Runtime Policy Pack and Governed Telemetry Adapter

The runtime evaluator (`AiGridRuntimeEvaluationService`) is a separate evaluation path from the artifact-facts loop above — its subject is an `EXECUTION`, not an artifact, and it reads the runtime metadata tables directly rather than `ai_grid_facts`. `AiGridAssessmentService` explicitly skips `EXECUTION`-subject policies (`isRuntimeMode`) so the two paths never double-evaluate the same policy. Three bounded evaluation modes, plus one bounded meta-mode, carry the platform V3 evaluation-mode vocabulary:

| Mode | Subject | Shape | Violation |
|---|---|---|---|
| `RUNTIME_FACTS` | one execution | a bounded conjunction (≤16) of execution-scoped conditions (`EQ`/`NE`/`IN`/`NOT_IN`/`EXISTS`/`ABSENT`) | every condition holds |
| `RUNTIME_SEQUENCE` | one execution | ordered event-scoped steps within a maximum duration and lateness, with an explicit dedup key | all steps match in order |
| `RUNTIME_AGGREGATE` | the anchoring execution of a group | a registered metric (retries, repeated action signature, tokens, latency, spend) over a fixed lookback window and an allowlisted grouping key, compared against a threshold | the metric breaches the threshold |
| `RUNTIME_COVERAGE` | one provider/workspace/capability-family/window | a bounded coverage meta-policy | collection is partial for that scope |

Field references resolve only against `platform.ai_grid_runtime_field_definitions`, fully qualified (`execution.*` vs `event.*` — the two scopes are never conflated even where a field shares a name, e.g. `approval_state`); an unregistered or JSONB-diagnostic field fails package validation before the policy can ship. An unpopulated governed field settles `UNKNOWN` (never `PASS`); an unsupported evidence scope or a stale required capability settles `NOT_ASSESSED`. Runtime `FAIL`s reconcile onto `finding_kind = AI_RUNTIME` (tenant V3), deduplicated by the sequence's dedup key or the aggregate's grouping key so repeated violations collapse onto one finding instead of one per occurrence.

**The policy pack (`policy-packages/agcf/AGCF-RT-*`, governed by `runtime-wave-contract.json`, validated by `scripts/validate-ai-grid-runtime-catalog.mjs`)** ships every package `PREVIEW`/`releaseStatus: PAUSED`, so — per the Phase 1 tenant-default fix above — a package only reaches `ENABLED` through an explicit tenant override, never by seeding:

| Policy | Wave | Mode | Fails when |
|---|---|---|---|
| AGCF-RT-001 | 2A | `RUNTIME_FACTS` | an execution cannot be resolved to exactly one agent and version |
| AGCF-RT-002 | 2A | `RUNTIME_FACTS` | an execution's version digest is absent from the tenant's approved manifest |
| AGCF-RT-003 | 2A | `RUNTIME_FACTS` | an execution terminates against a version the tenant has retired |
| AGCF-RT-004 | 2A | `RUNTIME_COVERAGE` | an execution's evidence source can't supply the decision context Wave 2B needs (one finding per provider/workspace/capability-family/window, never per execution) |
| AGCF-RT-005 | 2B | `RUNTIME_SEQUENCE` | a consequential action required approval and nevertheless succeeded |
| AGCF-RT-006 | 2B | `RUNTIME_SEQUENCE` | an enforcement point denied an action that nevertheless succeeded |
| AGCF-RT-007 | 2B | `RUNTIME_SEQUENCE` | a restricted/confidential read is followed by an external write or send with no approved declassification |
| AGCF-RT-008 | 2B | `RUNTIME_SEQUENCE` | an execution invokes a tool digest absent from the tenant's component allowlist |
| AGCF-RT-009 | 2B | `RUNTIME_AGGREGATE` | total retries across an agent's executions exceed the configured threshold in the lookback window |

Five further Wave 2C controls (`AGCF-RT-010`–`014` — unconstrained agent-to-agent handoff, unsafe durable-memory write, memory isolation/retention failure, tool/plugin/MCP integrity-provenance failure, missing stop/containment control) are declared `pausedPendingEvidence` in the same contract with no package files yet, each blocked on a named unsupported capability (`RUNTIME_HANDOFF_EVIDENCE`/`RUNTIME_MEMORY_EVIDENCE`(×2)/`RUNTIME_PROVENANCE_EVIDENCE`/`RUNTIME_CONTAINMENT_EVIDENCE`) and reporting `POLICY_PAUSED_PENDING_EVIDENCE` rather than a fictional per-subject assessment. Certification requires AGCF-RT-001 and -003 by name, at least two of -005/-006/-008, and five total — a count alone could be satisfied entirely by the easier Wave 2A correlation checks, so the behavioral minimum is enforced separately.

**The governed customer telemetry adapter** (`POST /api/internal/ai-grid/runtime/{producerId}/v1/batches`, `ROLE_SERVICE_ACCOUNT` + principal-equals-`producerId`, mirroring the existing evidence port) exists for frameworks, gateways, and enforcement points whose provider API cannot supply the decision evidence above. `AiGridRuntimeIngestionService` checks caps in the order they actually bind — 2 MiB request body first (the practical limit), then 100 executions per batch, then the existing per-execution event/metadata ceilings — and recursively rejects any forbidden raw-content field (prompt/response/argument/credential/document/message-body). A producer must be a registered, `ACTIVE` `ai_runtime_evidence_producers` row bound to the resolved tenant; a submitted `AUTHORITATIVE` evidence class is only honored once that producer's `certification_state = 'CERTIFIED'` — adapter evidence is otherwise admissible (any policy that declares the evidence class it accepts can use it, labeled by source) but never auto-promoted. Admission returns `202` with a receipt (readable back at `GET /api/ai-security/runtime-ingestion/receipts/{receiptId}`) or `429` with `Retry-After` on backlog or per-tenant quota exhaustion; `AiGridRuntimeIngestionWorker` (every 2s by default, one job per tenant per tick, reusing the existing `ingestion_jobs` infrastructure) then drains the batch through the same `AiAgentExecutionIngestionService` boundary the provider collectors use. Per-tenant quotas live in `ai_runtime_quota_windows` — a soft threshold at 80% of the configured window degrades with a signal while still accepting, a hard threshold rejects (`TENANT_QUOTA_EXHAUSTED`) and is never sampled or silently truncated, and a quota-exhausted window is marked as such so it can't pass a fill-rate gate on partially rejected data.

### Scheduled Jobs (AI Security / AI Grid)

| Cadence | Job |
|---|---|
| Every 3s (default) | `AiSecurityJobWorkerService` — poll `ingestion_jobs` for AWS, Azure, and Copilot Studio AI discovery jobs |
| Every 2s (default) | `AiGridRuntimeIngestionWorker` — poll `ingestion_jobs` for AI Grid runtime telemetry-adapter batches (one job per tenant per tick) |
...
| Every 5s (default) | `AiGridPolicyRolloutService` / `AiGridPolicyDeprecationService` — process pending GA/canary distribution and deprecation tasks |
...
- The runtime policy pack (`AGCF-RT-001`–`009`) is implemented and gate-green locally but has no live pilot tenant: no tenant currently has an Azure Foundry runtime endpoint or Copilot Studio configuration enabled, so no 14-day evidence window exists yet and the Program 2 entry gate has not actually been passed by any tenant. The configured Azure/Copilot metadata APIs also do not currently expose authoritative action-level decision records, so the event-decision families stay `NOT_ASSESSED` until a qualified provider export or a governed telemetry adapter producer supplies them.
- No representative customer answer-key corpus has been supplied for the runtime pack, so the certification requirement (AGCF-RT-001 and -003 by name, at least two of -005/-006/-008, five total) and any per-producer promotion of adapter evidence to `AUTHORITATIVE` remain pending — this is why every runtime package ships `PREVIEW`/`PAUSED` rather than `ENABLED`.
