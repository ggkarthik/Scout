...
**Run Queue (`InventoryRunQueuePage`, at `/connect/run-history`):** merges `useSyncRunsQuery`'s completed/active `SyncRun` rows with, for the async AI Security providers, in-flight `IngestionJob` rows from `useIngestionJobsQuery` while they're QUEUED, RUNNING, or FAILED (previously QUEUED-only, so a claimed/running job used to disappear from the queue). `hasReplacementSyncRun()` drops an ingestion-job row once a matching `SyncRun` for the same connector shows up, so a job doesn't double-render as it transitions from queued/running to a completed sync run. `formatSyncType` labels: `AI_SECURITY_AWS_BEDROCK` → "AWS AI Discovery", `AI_SECURITY_AZURE_DISCOVERY` → "Azure AI Discovery", `AI_SECURITY_COPILOT_STUDIO` → "Microsoft Copilot Discovery" — the last of these only ever arrives as a `SyncRun` (Copilot Studio discovery runs synchronously, not via `ingestion_jobs`; see `docs/business-logic-guide.md#ai-security--ai-grid-pipeline`). Triggering a run from `AiSecurityAzureConnectorPage` or `CopilotStudioConnectorPage` invalidates the `ingestion-jobs` and `sync-runs` query caches (in addition to the connector's own scoped `ai-security-runs` query) so the new run appears in this queue immediately rather than on the next poll tick.
...
- AI Grid governance (platform-scoped): `AiGridSystem`, `AiGridCoverage`, `AiGridPolicy`/`PolicySelection`/`PolicyDistribution`, `AiGridPolicyImpactPreview`, `AiGridPolicyReleaseReadiness`, `AiGridPolicyTenantReconciliation`, `AiGridPolicyCandidate`, `AiGridRunMetrics`
...
- Platform governance: `listPlatformAiGridPolicies()`, policy-ID `approvePlatformAiGridPolicy()`, `publishPlatformAiGridPolicy(policyId, targetTenantIds)`, `deprecatePlatformAiGridPolicy()`, `revokePlatformAiGridReleaseBinding()`, policy detail/readiness, framework coverage, `getPlatformAiGridPolicyCandidates()`
