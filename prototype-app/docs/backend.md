...
58 controllers total (44 under `com.prototype.vulnwatch.controller`, described in this section, plus 14 under the separate `com.prototype.vulnwatch.aisecurity.controller` package, described in its own [AI Security / AI Grid](#ai-security--ai-grid) section below), verified against `@RequestMapping`/`@GetMapping`/etc. annotations directly (several base paths below differ from older drafts of this doc — connector controllers moved under a shared `/api/connectors/` prefix at some point, and quite a few controllers have grown far beyond simple CRUD).
...
**AiGridValidationGovernanceController — `/api/platform/ai-grid/validation`** (`PLATFORM_OWNER` only) — answer-key/precision/bias review, policy-ID lifecycle, and R1/R2 release certification. `GET/POST /answer-keys[...]`, `POST /precision-reviews[...]`, `POST /policies/{id}/approve`, `POST /policies/{id}/publish`, `POST /policies/{id}/deprecate`, and `POST /release-bindings/{id}/revoke`; legacy version-keyed digest/readiness/publish routes return an explicit migration error. `GET/POST /releases/r1/*`, `GET/POST /releases/r2/*` remain available.
...
- **`TenantSchemaReadinessHealthIndicator`** — actuator `HealthIndicator`, only registered when `app.tenancy.enforce-schema-version=true`; reports `DOWN` with an `unreadyTenantCount` detail if any `ACTIVE` tenant is below `app.tenancy.minimum-compatible-schema-version` (default 1).
...
| `app.tenancy.minimum-compatible-schema-version` | `APP_MINIMUM_TENANT_SCHEMA_VERSION` | `1` | Minimum per-tenant schema version required for the readiness health check to report `UP` |
...
| Every 3 sec (default) | Poll `ingestion_jobs` for AI Security AWS/Azure discovery jobs (`AiSecurityJobWorkerService`) |
