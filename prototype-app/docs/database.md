...
| `target_version` | `integer` | NOT NULL; current packaged tenant target is `3` |
...
| `id` | `uuid` | PK |
| `tenant_id` | `uuid` | NOT NULL; FK → `platform.tenants(id)` |
| `job_type` | `varchar(80)` | NOT NULL; e.g. `SBOM_INGESTION`, `CONFIG_IMPORT`, `AI_SECURITY_AWS_BEDROCK`, `AI_SECURITY_AZURE_DISCOVERY` |
...
## AI Security / AI Grid Tables

Backs the `com.prototype.vulnwatch.aisecurity` module (19 controllers, 63 services — see `docs/business-logic-guide.md#ai-security--ai-grid-pipeline` and `docs/architecture.md`) and the `/findings/ai`, `/policies`, `/inventory/ai` frontend routes. Two generations coexist: a legacy **AI Security** catalog (hardcoded ~14-policy checklist) and the newer, platform-governed **AI Grid** pipeline that has superseded it for policy evaluation (legacy findings and policy selections were migrated into the canonical/governed tables). Unlike every other table in this document, **none of these tables have a JPA `@Entity` or Spring Data repository** — all reads/writes go through `JdbcTemplate`/`NamedParameterJdbcTemplate` in the `aisecurity.service` classes directly.
...
#### Runtime identity V4 → V5 upgrade

Before applying tenant V5, run `backend/scripts/preflight-ai-runtime-v5.sql`. V4 stored provider runtime identifiers in plaintext; V5 changes that boundary to tenant-scoped HMAC digests and therefore intentionally does not attempt an irreversible SQL-only conversion without the runtime key. If the preflight reports rows, pause the rollout and either export and re-ingest them through `AiAgentExecutionIngestionService` with the tenant runtime HMAC key, or—when runtime history is explicitly disposable—delete events, receipts, cursors, then executions for that tenant before rerunning the preflight. Inventory artifacts, findings, memberships, and discovery completeness must not be deleted as part of this cleanup.

**Shared AI inventory infrastructure** (`V45`, `V46`, `V62`–`V64`): `ai_security_connector_configs` (AWS Bedrock / Azure AI connector config, credentials encrypted), `ai_security_azure_credential_profiles` (client-secret/workload-federation/managed-identity, active + pending rotation), `ai_security_artifacts` (discovered AI resources — agents, models, guardrails, knowledge bases, MCP servers; carries PII scan status/source/info-types added in `V62`/`V64`), `ai_security_artifact_sources`, `ai_security_relationships`, `ai_security_snapshot_scopes`, and `ai_security_observation_receipts` (idempotency for ingested observation chunks). Tenant policy configuration and legacy findings were retired in `V65`; AI Grid and canonical findings are their sole owners.

**AI Grid core pipeline** (`V48`–`V57`): `ai_grid_snapshot_bodies`/`_manifests` (immutable, hash-deduped artifact snapshots), `ai_grid_facts` (versioned facts derived from snapshots — the input to policy evaluation), `ai_grid_artifact_classifications`, `ai_grid_policy_selections`/`_history` (tenant's REQUIRED/ENABLED/PREVIEW/DISABLED choice per governed policy), `ai_grid_assessments` (policy evaluation results: PASS/FAIL/NO_DECISION/ERROR/NOT_APPLICABLE), `ai_grid_coverage_gaps`, `ai_grid_systems`/`_system_revisions`/`_system_memberships` (agent-rooted AI system grouping with lineage), `ai_grid_system_lineage_events`/`_participants`, `ai_grid_system_membership_decisions`/`_overrides` (`V56`/`V57`), `ai_grid_outbox`, `ai_grid_owner_history` (ownership resolution audit trail), `ai_grid_run_metrics`/`_run_scope_metrics` (per-run economics/utility telemetry), `ai_grid_relationship_snapshots` (`V51`), `ai_grid_policy_readiness`, `ai_grid_setup_actions` (prioritized onboarding queue, `V52`), `ai_grid_current_coverage_state`/`_artifacts`, `ai_grid_current_expected_candidates` (materialized "current epoch" union of latest complete scans, `V53`). The core `findings` table gained `finding_subjects`/`finding_reviews` (`V48`) to generalize beyond CVE-shaped findings to `AI_POSTURE`/`AI_EXPOSURE` kinds.

**Tenant V2 runtime approvals:** `ai_grid_approved_agent_manifests` binds a tenant-approved agent version to an immutable manifest digest and component array; `ai_grid_component_allowlists` records approved/revoked provider component digests. Both tables have forced RLS and tenant-isolation policies. V2 also adds not-yet-validated check constraints for the normalized `ai_agent_executions.approval_state` and `policy_state` vocabularies. These objects exist only after that tenant has received V2 through the tenant schema control plane.

**Tenant V3 runtime contract:** widens `ai_agent_executions` with ~15 typed, policy-addressable columns — `environment_digest`, `deployment_digest`, `acting_identity_digest`, `delegated_identity_digest`, `identity_digest_key_version` (all HMAC-safe references, not raw identifiers), `termination_reason`, `evidence_source`, `evidence_class`, `evidence_confidence` (0–1, checked), `step_count` (≥0, checked), `spend_currency`/`spend_unit`, `collected_at`, `provider_event_time`, `delivery_latency_ms` (≥0, checked). Widens `ai_agent_execution_events` with the event-scoped decision contract — `producer_id`, `provider_event_digest`, `digest_key_version`, `ingested_at`, `action_category` (`READ`/`WRITE`/`DELETE`/`SEND`/`EXECUTE`/`ADMIN`/`PAYMENT`/`PUBLISH`/`OTHER`), `target_class` (`INTERNAL`/`EXTERNAL`/`PUBLIC`/`SENSITIVE_STORE`/`CODE_RUNTIME`/`IDENTITY_SYSTEM`/`FINANCIAL_SYSTEM`/`UNKNOWN`), `tool_digest`/`tool_version_digest`/`target_digest`/`action_correlation_digest`, `data_sensitivity` (`PUBLIC`/`INTERNAL`/`CONFIDENTIAL`/`RESTRICTED`/`UNKNOWN`), `data_operation` (`READ`/`WRITE`/`DELETE`/`TRANSFORM`/`TRANSMIT`/`NONE`/`UNKNOWN`), `approval_state` (`APPROVED`/`DENIED`/`REQUIRED`/`NOT_REQUIRED`/`BYPASSED`/`UNKNOWN`), `policy_state` (`ALLOWED`/`DENIED`/`BLOCKED`/`NOT_EVALUATED`/`UNKNOWN`), `decision_reason`, `enforcement_point`, `action_outcome` (`SUCCEEDED`/`FAILED`/`DENIED`/`BLOCKED`/`CANCELLED`/`UNKNOWN`), `evidence_class` — each a closed, uppercase, invalid-fails-ingestion enum. A partial unique index dedupes events on `(tenant_id, producer_id, provider_event_digest, digest_key_version)` when all three are present; a partial index on consequential `action_category` values speeds the Program 2 fill-rate queries. `ai_agent_execution_receipts.connector_id` becomes nullable because governed telemetry-adapter batches have no connector.

New tenant V3 tables: `ai_runtime_evidence_producers` (one row per registered adapter producer — `producer_id` PK, bound `tenant_id`, `service_account_id`, `provider`, `producer_type` restricted to `RUNTIME_EVIDENCE_PRODUCER`, `submitted_evidence_class`, `certification_state` `UNCERTIFIED`/`CERTIFIED`/`REVOKED`, `certified_evidence_families_json`, `status` `ACTIVE`/`INACTIVE`/`REVOKED`); `ai_runtime_source_configurations` (`source_id` PK, `source_kind` `PROVIDER_CONNECTOR`/`TELEMETRY_ADAPTER`, `required_for_program_gate` — seeded with `AZURE_FOUNDRY_RUNTIME` and `COPILOT_STUDIO_RUNTIME`, both `PROVIDER_CONNECTOR`/`required_for_program_gate=true`, read by `AiGridRuntimeTelemetryReadinessService` instead of a hardcoded provider list); `ai_runtime_quota_windows` (per tenant/source rolling window — accepted/rejected event and byte counts, `quota_state` `AVAILABLE`/`SOFT_LIMIT`/`EXHAUSTED`); `ai_runtime_ingestion_receipts` (per-batch admission outcome — `status` `ACCEPTED`/`PROCESSING`/`COMPLETED`/`REJECTED`/`FAILED`, accepted/duplicate/quarantined counts, `reason_code`, linked `ingestion_job_id`, read back via `GET /api/ai-security/runtime-ingestion/receipts/{receiptId}`). All four are RLS-forced with a `tenant_isolation` policy. `ai_grid_budget_config` gains `runtime_quota_window_seconds`/`rolling_runtime_event_limit`/`rolling_runtime_byte_limit`. `findings.finding_kind` widens to add `AI_RUNTIME` — the runtime evaluator's own finding kind, distinct from `AI_POSTURE`/`AI_EXPOSURE`. `ai_grid_assessments`' uniqueness moves from `(tenant_id, run_id, policy_id, subject_type)` to `uq_ai_grid_assessments_evaluation_identity` `(tenant_id, run_id, policy_id, subject_type, subject_id, fingerprint)` so a runtime evaluation's per-execution/per-group subjects can each hold their own row.

**Budget, retention, and evidence trust** (`V50`, `V54`, `V55`, `V59`): `ai_grid_budget_config` (daily scan/API-call/byte/processing limits, seeded OBSERVE mode), `ai_grid_scan_cadence_rules`, `ai_grid_budget_admissions`/`_alerts` (`V54` adds environment/criticality scope), `ai_grid_retention_policies` (seeded HOT/ARCHIVE/RESTRICTED_EVIDENCE classes), `ai_grid_evidence_holds`, `ai_grid_retention_decisions`, `ai_grid_retention_purge_audit` (`V55` — durable audit surviving snapshot-body deletion). `ai_grid_host_context_facts` (`V56`; `producer_id` added `V59`) holds CIEM/DSPM/ASM/runtime evidence from trusted external producers, ingested via `AiGridEvidenceIngestionController` (`ROLE_SERVICE_ACCOUNT` only) — and, since the AWS connector-maturity work, the `activity.*` facts from the `SCOUT_AWS_ACTIVITY` producer. Those rows are queried by fact-key prefix (`fact_key like 'activity.%'`) by both `GET /api/ai-security/artifacts/{id}/activity-evidence` and the exposure-priority rollup, and are filtered on `state='KNOWN' and valid_until > now()`, so expired evidence stays auditable without contributing to current activity state.

**R2 exposure correlation** (`V56`, `V57`): `ai_grid_exposure_paths`/`_observations`/`_associations`/`_dispositions` (cross-system exposure hypotheses promoted to "validated" only on exact, freshness-checked facts; disposition = accept/false-positive), `ai_grid_exposure_executions` (binds a correlation result to the exact material/epoch that produced it, for deterministic replay verification).

### Platform schema tables (pre-reset provenance: `V47`–`V73`)

Governance-only — no tenant data. `platform.ai_grid_policy_distribution` (`V67` provenance; rollout stage GA/canary/paused/retired, canary tenant list, pinned version) is the sole policy-distribution control. `ai_grid_technology_versions`, `ai_grid_fact_definitions`, `ai_grid_policy_versions` (`V50`, `V51`, `V70` provenance; the immutable policy package catalog — lifecycle state, `native_kinds_json`/`scope_resolution`, `parameter_definitions_json`) and `ai_grid_correlation_versions`/`_correlation_precision_reviews` hold canonical correlation templates. Release/certification governance uses `ai_grid_answer_key_environments`/`_cases`/`_runs`/`_results`, precision-review tables, release decisions, gate evidence, and immutable release manifest items. `ai_grid_policy_candidates` is the RICE-style intake backlog for future policy authoring. The one-time AI Grid reset log/service is not present in the clean-start baseline.

**Platform V2 framework contract:** `platform.ai_grid_frameworks` and `platform.ai_grid_framework_controls` are the authoritative registry for OWASP GenAI LLM Top 10 2026, OWASP Agentic Top 10 2026, and CSA AICM 1.1 reporting. `platform.ai_grid_runtime_field_definitions` is the allowlist of typed execution/event columns that runtime predicates may reference; diagnostic/extension JSON is intentionally absent. The immutable SQL function `platform.ai_grid_policy_visible_to_tenant(available, rollout_stage, canary_ids, tenant_id)` centralizes the GA or tenant-listed CANARY/DEV visibility rule used by catalog and coverage queries.

**Platform V3 runtime evaluation contract:** widens `ai_grid_policy_versions.evaluation_mode` to add `RUNTIME_FACTS`, `RUNTIME_SEQUENCE`, `RUNTIME_AGGREGATE`, and the bounded meta-mode `RUNTIME_COVERAGE`; widens `evaluation_subject` to add `EXECUTION`. Adds `physical_column` (boolean) to `ai_grid_runtime_field_definitions` and ~19 new `execution.*`/`event.*` field rows sourced from the tenant V3 columns above (three — `execution.version_approval_state`, `execution.version_lifecycle_state`, `event.component_approval_state` — are derived/non-physical). Inserts five new `ai_grid_resource_family_definitions` rows (`RUNTIME_EXECUTIONS`, `RUNTIME_EVENT_DECISIONS`, `RUNTIME_COMPONENT_USE`, `RUNTIME_DATA_HANDLING`, `RUNTIME_AUTONOMY_CONTROLS`) and matching `ai_grid_capability_definitions` rows (`RUNTIME_EXECUTION_METADATA`, `RUNTIME_VERSION_CORRELATION`, `RUNTIME_EVENT_DECISIONS`, `RUNTIME_ACTION_OUTCOMES`, `RUNTIME_TOOL_CORRELATION`, `RUNTIME_DATA_CLASSIFICATION`, plus four `optional=true` wave-2C capabilities — `RUNTIME_HANDOFF_EVIDENCE`, `RUNTIME_MEMORY_EVIDENCE`, `RUNTIME_PROVENANCE_EVIDENCE`, `RUNTIME_CONTAINMENT_EVIDENCE` — that stay unsupported pending an authoritative evidence source). These rows are what let an unsupported runtime evidence family report a named `NOT_ASSESSED` capability gap instead of silently passing.
...
- **Never edit an already-applied migration file.** Add a new migration instead. (Before the one-time migration-history reset, a narrow exception existed for two files needing a schema-qualification fix — that fix is now simply part of the `V1` baselines, so no live exception currently applies; don't treat historical precedent as license to edit an applied migration today.)
- V1 baseline DDL uses defensive creation patterns; later migrations may contain one-time `ALTER TABLE` operations and are not assumed idempotent outside Flyway.
- `flyway.validate-on-migrate: true` in all profiles; `out-of-order: false`.
- `flyway.baseline-on-migrate: false` in all profiles (`application.yml`) — there is currently no local-profile override (only a gitignored `application-local.example.yml` template exists, and it has no Flyway section).
