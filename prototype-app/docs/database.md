...
Backs the `com.prototype.vulnwatch.aisecurity` module (11 controllers, 37 services — see `docs/business-logic-guide.md#ai-security--ai-grid-pipeline` and `docs/architecture.md`) and the `/findings/ai`, `/policies`, `/inventory/ai` frontend routes. Two generations coexist: a legacy **AI Security** catalog (hardcoded ~14-policy checklist) and the newer, platform-governed **AI Grid** pipeline that has superseded it for policy evaluation (`V60`/`V61` migrate legacy findings and policy selections into the canonical/governed tables). Unlike every other table in this document, **none of these tables have a JPA `@Entity` or Spring Data repository** — all reads/writes go through `JdbcTemplate`/`NamedParameterJdbcTemplate` in the `aisecurity.service` classes directly.

### Tenant schema (`tenant/V45`–`V64`)

**Legacy AI Security bounded context** (`V45`, `V46`, `V58`, `V62`–`V64`): `ai_security_connector_configs` (AWS Bedrock / Azure AI connector config, credentials encrypted), `ai_security_azure_credential_profiles` (client-secret/workload-federation/managed-identity, active + pending rotation), `ai_security_artifacts` (discovered AI resources — agents, models, guardrails, knowledge bases, MCP servers; carries PII scan status/source/info-types added in `V62`/`V64`), `ai_security_artifact_sources`, `ai_security_relationships`, `ai_security_snapshot_scopes`, `ai_security_observation_receipts` (idempotency for ingested observation chunks), `ai_security_policy_settings`/`_scopes`/`_artifact_overrides`/`_parameters` (per-tenant policy targeting, added `V58`), `ai_security_findings`/`_finding_reviews` (superseded by the canonical `findings` table as of `V60`, kept as legacy source until then).
...
### Platform schema (`postgres_reset/V47`–`V73`)

Governance-only — no tenant data. `platform.ai_security_policy_distribution` (`V48`; legacy policy availability/default-enabled toggle, seeded from AWS Bedrock + Azure AI/ML/Search/Bot policies) and its successor `platform.ai_grid_policy_distribution` (`V67`; rollout stage GA/canary/paused/retired, canary tenant list, pinned version — the canonical distribution control as of `V67`, kept in sync with the legacy table during migration). `ai_grid_technology_versions`, `ai_grid_fact_definitions`, `ai_grid_policy_versions` (`V50`, `V51`, `V70`; the immutable policy package catalog — lifecycle state, `native_kinds_json`/`scope_resolution`, `parameter_definitions_json`) and `ai_grid_correlation_versions`/`_correlation_precision_reviews` (`V63`; the three R2 correlation templates). Release/certification governance: `ai_grid_answer_key_environments`/`_cases`/`_runs`/`_results` and `ai_grid_precision_reviews`/`_samples`/`_labels`/`_adjudications` (`V53`, provenance columns `V56`) — a labeled test-case + statistical (Wilson-interval) precision review framework gating what reaches PUBLISHED; `ai_grid_policy_release_decisions`, `ai_grid_release_gate_evidence`/`_release_decisions` (`V59`, digest column `V61`), `ai_grid_release_manifest_items` (`V65` — immutable, UPDATE/DELETE blocked by trigger). `ai_grid_policy_candidates` (`V69` — RICE-style intake backlog for future policy authoring). `ai_grid_test_data_reset_log` (`V71` — singleton audit of a deliberate, irreversible one-time test-data wipe).

### Known anomalies (flagged during the 2026-08-18 documentation audit, not yet remediated)

- `postgres_reset/V46__demo_request_active_email_uniqueness.sql` (unrelated to AI Grid — a demo-request email-uniqueness fix) is missing the required `-- migration-guard: platform-only` header comment, and its DML reaches into `tenant_default.demo_requests` by schema-qualified name from the platform-only line. Every other file in `V45`–`V73` has the guard comment.
- The platform line's tracked tenant-schema `target_version` (in `platform.tenant_schema_versions`, last bumped in `V68__ai_grid_policy_migration_target.sql`) is **61**, while the tenant line's actual latest file is **V64** (`V62`–`V64`: PII classification, Purview account config, unknown-sensitivity backfill). This may be intentional staged rollout rather than drift — confirm with the AI Grid control-plane owner before treating it as a bug.
