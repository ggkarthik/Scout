...
Backs the `com.prototype.vulnwatch.aisecurity` module (14 controllers, 48 services — see `docs/business-logic-guide.md#ai-security--ai-grid-pipeline` and `docs/architecture.md`) and the `/findings/ai`, `/policies`, `/inventory/ai` frontend routes. Two generations coexist: a legacy **AI Security** catalog (hardcoded ~14-policy checklist) and the newer, platform-governed **AI Grid** pipeline that has superseded it for policy evaluation (legacy findings and policy selections were migrated into the canonical/governed tables). Unlike every other table in this document, **none of these tables have a JPA `@Entity` or Spring Data repository** — all reads/writes go through `JdbcTemplate`/`NamedParameterJdbcTemplate` in the `aisecurity.service` classes directly.

**A note on the version numbers below:** a one-time migration-history reset consolidated the entire numbered migration line into two fresh baselines, `postgres_reset/V1__platform_schema.sql` and `tenant/V1__tenant_schema.sql` (see "Tenant Schema Control Plane" further down). Every `V<n>` citation in this section describes a table's *pre-reset provenance* — useful for cross-referencing old PRs/commit history — not a currently addressable file. All of these tables now live in the single `V1` baseline for their line.
...
Governance-only — no tenant data. `platform.ai_grid_policy_distribution` (`V67`; rollout stage GA/canary/paused/retired, canary tenant list, pinned version) is the sole policy-distribution control. V67 performed a one-time compatibility backfill into the now-retired legacy distribution table; it was not a live dual-write. `ai_grid_technology_versions`, `ai_grid_fact_definitions`, `ai_grid_policy_versions` (`V50`, `V51`, `V70`; the immutable policy package catalog — lifecycle state, `native_kinds_json`/`scope_resolution`, `parameter_definitions_json`) and `ai_grid_correlation_versions`/`_correlation_precision_reviews` (`V63`; the three R2 correlation templates). Release/certification governance: `ai_grid_answer_key_environments`/`_cases`/`_runs`/`_results` and `ai_grid_precision_reviews`/`_samples`/`_labels`/`_adjudications` (`V53`, provenance columns `V56`) — a labeled test-case + statistical (Wilson-interval) precision review framework gating what reaches PUBLISHED; `ai_grid_policy_release_decisions`, `ai_grid_release_gate_evidence`/`_release_decisions` (`V59`, digest column `V61`), `ai_grid_release_manifest_items` (`V65` — immutable, UPDATE/DELETE blocked by trigger). `ai_grid_policy_candidates` (`V69` — RICE-style intake backlog for future policy authoring). `ai_grid_test_data_reset_log` (`V71` — singleton audit of a deliberate, irreversible one-time test-data wipe).

### Known anomalies

Both anomalies previously logged here (a pre-reset platform-only file missing its guard comment, and a `target_version`/actual-latest-file mismatch between `61` and `V64`) were artifacts of the pre-reset numbered migration line and no longer apply: the current `postgres_reset/V1__platform_schema.sql` does carry the `-- migration-guard: platform-only` header, and both Flyway lines now resolve their latest version as `1` via `PackagedMigrationCatalog`.

A related, *not yet verified* concern surfaced during the post-reset documentation audit: `app.tenancy.minimum-compatible-schema-version` still defaults to `53` (`application.yml`, `application-prod.yml`), while every tenant schema migrated after the reset lands on `target_version = 1`. If that reproduces against a live DB, a freshly-provisioned tenant would report as below the minimum-compatible version — worth confirming with the tenant-schema control-plane owner before assuming it's fine, since a default of `53` is likely stale in the same way the file-numbering was.
