...
| `target_version` | `integer` | NOT NULL; current clean-start target is `1` |
...
The clean-start release has no post-baseline migrations. Current heads are platform `postgres_reset/V1` and tenant `tenant/V1`, both pinned by `MigrationCatalogTest`. AWS connector capabilities, R2 v2 correlations, governed activity facts, capability manifests, and scope-owned artifact attributes are part of those V1 baselines.
...
`app.tenancy.minimum-compatible-schema-version` now defaults to `1`, matching the packaged clean-start tenant target.
